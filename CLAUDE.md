...
The north-star metric is the **Pre-Action Catch Rate**: the share of defects stopped *before* a human or a tool acted on them. Targets the implementation must be measured against from the start: catch rate ≥ 90%, added p95 latency ≤ 120 ms (time-to-first-token statistically unchanged), verification cost ≈ 6% of model spend, net spend change ≈ −15%, ungrounded escapes ≤ 1% (conformally bounded at 90% confidence).
