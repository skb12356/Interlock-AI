...
- `[~]` **D2-A4 — `gate/repair.py` (L2)** — *the gate's repair path is built and tested (two attempts then escalate; a failed repair withholds rather than shipping the original). Remaining: the actual re-prompt against the model.* — original scope:
