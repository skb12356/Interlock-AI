...
**North-star metric:** Pre-Action Catch Rate ≥ 90%. **Release targets, measured not aspirational:** catch ≥ 90% · added p95 ≤ 120 ms (TTFT unchanged) · verification cost ≈ 6% of model spend · net spend ≈ −15% · ungrounded escapes ≤ 1% (conformal, 90% confidence).
...
- `[x]` **D3-B7 — `eval/` harness + seeded set v1** — *done 2026-08-26. 200 conversations, exact category counts asserted by a test, **140 clean on purpose** (intervention behaviour can only be measured against traffic that deserved no intervention). Paired design: identical generations in both arms, so every difference is Interlock's. `make eval` prints the metric set. **Every release target passes; clean high-stakes traffic is still intervened on** -- see F-019.* — original scope: — 200 conversations, **60 induced failures**: 15 missing-retrieval, 10 number corruptions, 10 poisoned docs, 8 canary/PII, 10 demographic twin pairs, 7 loop-inducing agent tasks; each machine-checkable. `make eval` runs **off vs on** and prints the full metric set.
