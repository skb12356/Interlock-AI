...
- `[x]` `reliability.png` + `lambda.json` committed with a certified (α=0.01, δ=0.10) — *recalibrated 2026-08-30 at threshold 0.3700 on n=840 with a 9.22% intervention rate and 0.36% empirical escape rate*
...
- `[~]` **D4-B6 — Seeded set v2** — *the leakage half done 2026-08-26: calibration and eval are now **provably disjoint by document and stratified by domain**, with a test asserting it end-to-end. The detector repair and 2026-08-30 recalibration reduced verification cost to 0.48%-0.78% across three seeds. Remaining: expand the failure taxonomy with naturally occurring defects.* — expand the failure taxonomy; **fix leakage: the calibration set and the eval set must be provably disjoint, and you must be able to say so.**
...
- `[x]` **D5-B1 — The measurement run** — *regenerated 2026-08-30 after F-019. `scripts/build_eval_report.py` runs the full 200-case off-vs-on evaluation across three seeds and writes `artifacts/eval/report.html` plus per-seed JSON with Wilson intervals. Catch remains 100% and empirical ungrounded escapes 0% on all seeds. Verification cost is 0.78/0.48/0.62%, net spend -22.56/-20.67/-20.81%, and false interventions are 0/140 on each seed. The false-intervention point target passes, but its 95% upper bound is 2.67%; net spend remains the scorecard miss.*
...
- `[x]` **D5-B3 — `docs/LIMITATIONS.md`** — *updated 2026-08-30. Covers the remaining net-spend miss, the 0/140 false-intervention confidence limit, F-019 policy, assumed efficacy, optional observer, provenance over-blocking, single-vertical scope, empty Lane C live sample set, and the deterministic-upstream rehearsal caveat.*
