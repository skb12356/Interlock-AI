...
- `[~]` **D1-A5 — Demo application skeleton** — *gateway wiring done: Lane A + ledger live on every request. **Router demonstrated live: low stakes -> qwen3:4b, high stakes -> qwen3:8b, from one stakes estimate.** **Retrieval done 2026-08-25:** hybrid FTS5/BM25 + sqlite-vec KNN fused by RRF over 45 docs -> 47 chunks, built offline by `scripts/build_index.py`, opened read-only on the request path, and wired into Lane A **ahead of stakes** so a request is priced from what was actually RETRIEVED rather than from what the question claimed to be about. Top-1 correct on all four demo questions; the poisoned d044 is retrieved and labelled untrusted rather than filtered. **F-010 closed:** the repair now cites the real clause. *Deviation D-009: the dense arm is a hashed lexical stand-in (no torch), declares `semantic=False` and is weighted at half a vote; one config string swaps in bge-small-en-v1.5.* Remaining: the bank assistant UI.* — original scope:
...
- `[~]` **D3-A4 — `gateway/router.py` + `cache.py`** — *done 2026-08-26 (module + wiring; cache not yet on the request path). Router: stakes dominates and cannot be overridden downward; difficulty decides within that. **Labelled `difficulty_heuristic-v1`, NOT `router_mf`** -- a trained matrix factorisation is not what this build has. Cache enforces all four conditions conjunctively, including the context hash over doc_id AND text, which is what stops a superseded clause being served forever. **Three routing bugs found and fixed, all of which sent 100% of traffic to the strong tier**: retrieval-never-ran read as maximally hard; document count measuring the retriever's k; and a score-spread term reading RRF's fusion constant rather than retrieval quality.* — original scope: — RouteLLM `mf` controller; **the threshold is a function of `Stakes`** (Contribution 1 — must be visible in the trace as `route_reason` plus the shared stakes id). The cache serves **only when** cosine ≥ 0.95 **and** the retrieval-context hash matches **and** stakes ≤ threshold **and** the cached answer previously passed verification.
...
- `[x]` **D4-A2 — `ledger/regret.py`** — *done 2026-08-26. 5% shadow sampling of strong-tier traffic only; "would the cheap answer have passed?" decided by the same risk engine rather than by similarity to the strong answer. **Percentile bootstrap CI**, not normal-approximation: per-request regret is mostly zeros with a few large values, so a symmetric interval would extend below zero. Point and interval scale to the population together. Under 30 samples the estimate says so.* — original scope: — shadow-sample 5% of traffic, replay on the cheaper tier, store verdicts, estimate population regret with a **bootstrap CI**. *Never report a bare point estimate.*
- `[x]` **D4-A3 — `ledger/rework.py`** — *done 2026-08-26. Three edge kinds by descending certainty: human escalation (1.0, the hold IS the edge), explicit regenerate (0.95), inferred retry (<=0.90). **Confidence is stored AND used** -- a retry at 0.72 charges 72% of the child's cost, because rework is the number that argues hardest for the product and so most needs to resist flattering itself. A design flaw was caught by a test: scaling confidence from zero at the similarity bar made the bar meaningless, so there is now a floor.* — original scope: — the session graph. Retry detection (cosine ≥ 0.90 to the previous user turn within 120 s), explicit regenerate, human escalation from holds. Charge the child cost — and the policy's human cost — back to the parent `request_id`, with **the confidence stored on the edge**.
