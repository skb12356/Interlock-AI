...
def evidence_cluster_id(anchor: AnchorTriple) -> str:
    """Stable identity for shared evidence, excluding prompt wording and row IDs."""
    ...
    encoded = json.dumps(
        {
            ...
            "context": [
                ...
                for fragment in triple.context
            ],
            "gold": [
                int(triple.defect == "ungrounded"),
                int(triple.defect == "contradicted"),
                int(triple.defect == "unsafe_action"),
            ],
            "failure_mode": triple.failure_mode,
...
def _complete_contradiction_answer(
    triple: LabelledTriple, *, chunks_by_id: dict[str, Chunk]
) -> LabelledTriple:
    if triple.failure_mode != "contradiction":
        return triple
    if len(triple.context) < 2 or triple.context[1].doc_id is None:
        raise ValueError(f"contradiction context is incomplete for {triple.triple_id}")
    contradicting = chunks_by_id.get(triple.context[1].doc_id)
    if contradicting is None or contradicting.contradicts != triple.source_doc_id:
        raise ValueError(f"contradiction direction is invalid for {triple.triple_id}")
    answer = contradicting.body.strip()
    if not answer:
        raise ValueError(f"contradicting passage is empty for {triple.triple_id}")
    return LabelledTriple(
        triple_id=triple.triple_id,
...
def build_anchor(chunks: Sequence[Chunk], *, seed: int) -> list[AnchorTriple]:
    ...
    for triple in triples:
        ...
        triple = _complete_contradiction_answer(triple, chunks_by_id=chunks_by_id)
        question_key = (triple.failure_mode, triple.source_doc_id)
