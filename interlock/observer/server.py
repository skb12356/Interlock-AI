        ...
        try:
            ...
            if "claims" in request.want and runtime.verifier is not None:
                verdict = await asyncio.to_thread(
                    runtime.verifier.verify, request.sentence, context
                )
                claims = [
                    ClaimVerdict(
                        text=item.text,
                        label=item.label if item.label != "unjudged" else "unfindable",
                        span=item.span,
                    )
            ...
            runtime.failures += 1
            return ObserveResponse.degraded_response(
                f"observer inference failed: {type(exc).__name__}"
            )
        finally:
