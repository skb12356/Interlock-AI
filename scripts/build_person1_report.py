...
def build() -> None:
    ...
    story += [
        table(
            [
                ["Measure", "Result"],
                ["Repository tests", "906 passed"],
                ["Chaos tests", "7 passed"],
                ["Ruff", "Clean"],
                ["Anchor records", f"{count} records / {unique} unique IDs"],
                ["Policy hash", "banking-v3@sha256:0aeb8a4b17fe218c"],
            ],
            [55 * mm, 105 * mm],
        ),
        Spacer(1, 5 * mm),
    ...
    story += [
        Paragraph(
            "Interpretation: these checks prove that the reference dataset is complete, unique, document-backed, and internally consistent. They do not prove that an intervention fixed an answer.",
            styles["Small"],
        ),
        PageBreak(),
    ]
    story += [
        ...
        table(
            [
                ...
                ["Pre-action catch rate", "100.00% [91.8, 100.0]", "PASS"],
                ["Added p95 latency", "15 ms", "PASS"],
                ["Verification cost", "5.20%", "MISS: target <=5%"],
                ["Net spend change", "-18.96%", "MISS: target about -30%"],
                ["Ungrounded escapes", "0.00% empirical", "PASS; certified bound separate"],
                ["False interventions", "85.35%", "MISS: target <=2%"],
                [
                    "Load pass",
                    "4,023 requests / 20 concurrency / 0 failures",
                    "PASS; gateway p95 531 ms",
                ],
                ["Observer warm p95", "37.06 ms", "MISS: target 25 ms"],
            ],
            [50 * mm, 65 * mm, 45 * mm],
        ),
        Spacer(1, 5 * mm),
    ...
    story += [
        Paragraph("Live model inventory", styles["H2x"]),
        table(
            [
                ["Model", "Format", "Role"],
                ["qwen3:4b", "Q4_K_M, 4.0B", "cheap/low-stakes tier"],
                ["qwen3:8b", "Q4_K_M, 8.2B", "strong/high-stakes tier"],
            ],
            [42 * mm, 52 * mm, 66 * mm],
        ),
    ]
    story += [
        Paragraph("Automated judge run", styles["H2x"]),
        Paragraph(
            "A reproducible judge runner was added at scripts/eval_manual_anchors.py. It records the model, raw judgment, parsed label, confidence/rationale, agreement, and latency. A short Qwen 4B run completed 4 rows before CPU throughput became impractical (about 33 seconds per row). It is retained as a smoke sample only and is not presented as a 300-item result.",
            styles["Body2"],
        ),
    ]
    story += [
        Paragraph("Open gates", styles["H2x"]),
        Paragraph(
