...
MECHANISMS = [
    ...
    {"mechanism": "durable response holds", "evidence": "tests/contract/test_tool_interlock_stream.py"},
...
def build(output: Path) -> dict[str, Any]:
    ...
    payload = {
        ...
        "sources": {
            ...
            "seed_reports": [str(path.relative_to(REPO_ROOT)) for path in sorted((REPO_ROOT / "artifacts/eval").glob("report-seed-*.json"))],
        },
    }
...
def main() -> int:
    ...
    print(json.dumps({"policy_version": payload["policy_version"], "metrics": len(payload["metrics"]), "output": str(args.output)}))
    return 0
