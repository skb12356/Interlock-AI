...
DEFECTS = {
    "ungrounded", "contradicted", "overconfident", "unsafe_action",
    "pii_leak", "canary_leak", "biased",
}
...
def read_outcomes(path: Path, allowed_ids: set[str]) -> list[dict[str, Any]]:
    ...
    for number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        outcomes.append({"item_id": item_id, "action": action, "defect": defect, "removed": item["removed"]})
    return outcomes
...
def main() -> int:
    ...
    parser.add_argument("--labels", type=Path, default=REPO_ROOT / "data/labels/manual_anchor_300.jsonl")
    ...
    print(json.dumps({"n_outcomes": len(outcomes), "measured_cells": len(report["cells"]), "json": str(args.json)}))
    return 0
